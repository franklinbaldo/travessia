...
const manuscritoCollection = defineCollection({
  loader: glob({ pattern: '**/*.md', base: pathToFileURL(path.join(__dirname, '../../../manuscrito')) }),
  schema: z.object({
    titulo: z.string(),
...
const bastidoresRiobaldoCollection = defineCollection({
  loader: glob({ pattern: '**/*.md', base: pathToFileURL(path.join(__dirname, '../../../.jules/riobaldo')) }),
});

const bastidoresTedCollection = defineCollection({
  loader: glob({ pattern: '*-journal.md', base: pathToFileURL(path.join(__dirname, '../../../.jules/ted')) }),
});

const docsCollection = defineCollection({
  loader: glob({ pattern: '{events-all-the-way-down,riobaldo-blueprint}.md', base: pathToFileURL(path.join(__dirname, '../../../.jules/ted')) }),
});
