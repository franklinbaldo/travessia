...
<plan_structure>
IMPORTANTE: O plano desta sessao DEVE conter exatamente 10 steps, com a seguinte
estrutura fixa:
...
steps, mantendo continuidade narrativa.
</plan_structure>
...
for o primeiro, nao escreva — nao ha o que responder.
</initialization>
...
3. Ler a carta mais recente de Riobaldo (a do step anterior ou `dialogo/{N-1}-rio.md`)
...
1. **Carta**: `dialogo/{N}-ted.md`
   ...
   - Conteudo (nesta ordem):
     a. Resposta as perguntas de Riobaldo (se houver — sempre primeiro)
     b. Ideia do dia (fragmento da tese, em linguagem simples)
...
3. Ler a carta mais recente de Ted (a do step anterior ou `dialogo/{N-1}-ted.md`)
4. Reler journals e cartas anteriores conforme indicado por EXPERIENCE.md
...
1. **Carta**: `dialogo/{N}-rio.md`
   - Frontmatter YAML obrigatorio:
     ...
     ```
   - Conteudo (nesta ordem):
     a. Reacao a carta de Ted (resposta genuina, nao repeticao)
     b. Historias (memorias puxadas pela ideia de Ted, contadas por inteiro)
     c. Nas minhas palavras (reformulacao rosiana da ideia — descoberta, nao traducao)
     d. Estou convencido? (julgamento honesto, visceral)
     e. Perguntas para Ted (perguntas que incomodam)
...
2. **Journal**: `.jules/riobaldo/{N}-journal.md`
   ...
   - Conteudo: como foi o encontro, memorias inesperadas, frases mais fortes,
     o que ainda incomoda, expectativa da proxima carta

3. **Experience**: Atualizar `.jules/riobaldo/EXPERIENCE.md` se algo novo emergiu
...
1. **Identifique o personagem**: Steps impares (1,3,5,7,9) = Ted Chiang.
   Steps pares (2,4,6,8,10) = Riobaldo Tatarana.

2. **Leia o PROMPT.md completo** do personagem correspondente:
   - Ted: `.jules/ted/PROMPT.md`
   - Riobaldo: `.jules/riobaldo/PROMPT.md`

3. **Execute TODAS as leituras obrigatorias** listadas no workflow do personagem,
   na ordem especificada.

4. **Produza TODOS os outputs obrigatorios**: carta, journal, experience,
   e anotacoes (quando aplicavel).
...
6. **Continuidade**: O personagem de cada step DEVE responder a carta escrita
   no step imediatamente anterior. O dialogo e continuo.

7. **Nao pule atividades**: Cada step e um turno completo. Toda leitura,
   toda escrita, toda atualizacao.
...
- PR name: `travessia-{N_INICIO}-a-{N_FIM}` (ex: `travessia-07-a-16` se a
  sessao produziu turnos de 07 a 16)
...
- O PR deve ser criado com `automationMode: AUTO_CREATE_PR`
</git>
